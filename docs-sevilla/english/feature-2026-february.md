...
2. On the streak calendar screen (`/calendario-rachas`), a highlighted card informs them they have a daily routine ready. If they have already completed the routine that day, a green success card is shown confirming their training is complete.
...
### 2. `pialara/templates/audios/calendario_rachas.html` (Calendar View)
* Added a UI element `<div class="card text-center border-primary shadow-sm w-100">` with an inviting Call-To-Action (CTA) to use the automated generator.
* Implemented a conditional block evaluating the `racha_completada_hoy` session variable to display a green success card with localized texts and a disabled button if the routine is already complete.
...
### 5. `pialara/models/User.py` & `pialara/db.py` (Database)
* Extended the `User` model in `User.py` and updated the `get_user` functions in `db.py` to ensure the `ultima_rutina` attribute is loaded and bound to the session from MongoDB. This separates the "last time any audio was recorded" state from the "last time the required routine was completed" state.

### 6. Translation Files (`pialara/translations/`)
* Using Babel, the new strings from the conditional success card were extracted, updated, and compiled into new dictionary entries in `messages.po` and `messages.mo` (English and Spanish).
